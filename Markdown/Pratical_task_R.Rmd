...
Before we actually start, we have to load the packages necessary for the reading in the data and manipulating it. The `tidyverse` package collection provides a variety of tools from packages like `dplyr` and `tidyr`, `readxl` provides a robust method to directly read from Excel files, such as `.xls` and `.xlsx` files, and `lubridate` provides intuitive tools for handling of dates and times. To access the API we need functions from the `httr` package, and eventually we need to fit generalized linear models (GAM) from the `mgcv` package. `sf` helps in the graphical display of the measurement sites. 
```{r}
library(tidyverse)
library(readxl)
library(lubridate)
...
# 3 Write the new file into an Excel file and export it to an SQLite database

## 3.1 Calculate average, minimum and maximum chart datum in SQLite
...
Looks like everything is there. We now need a sufficiently good map of Norway, ideally in `.shp` format. Luckily the Europen Environment Agency [offers such a map](https://www.eea.europa.eu/data-and-maps/data/eea-reference-grids-2/gis-files/norway-shapefile/at_download/file "Norway shapefile"). Once downloaded with `download.file()` and unzipped with `unzip()`, the files can be used and adequatly manipulated. *Note that the download and unzipping part has been commented out*
```{r}
